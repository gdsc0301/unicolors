import {ColorSet, ColorPatterns} from './modules/colorSet';
// a
export default class UniColors {
    public XTerm:ColorSet = {
        Black: {
            ID: 0,
            hex: "#000000",
            rgb: {
                r: 0,
                g: 0,
                b: 0
            },
            hsl: {
                h: 0,
                s: 0,
                l: 0
            },
            name: "Black"
        },
        Maroon: {
            ID: 1,
            hex: "#800000",
            rgb: {
                r: 128,
                g: 0,
                b: 0
            },
            hsl: {
                h: 0,
                s: 100,
                l: 25
            },
            name: "Maroon"
        },
        Green: {
            ID: 2,
            hex: "#008000",
            rgb: {
                r: 0,
                g: 128,
                b: 0
            },
            hsl: {
                h: 120,
                s: 100,
                l: 25
            },
            name: "Green"
        },
        Olive: {
            ID: 3,
            hex: "#808000",
            rgb: {
                r: 128,
                g: 128,
                b: 0
            },
            hsl: {
                h: 60,
                s: 100,
                l: 25
            },
            name: "Olive"
        },
        Navy: {
            ID: 4,
            hex: "#000080",
            rgb: {
                r: 0,
                g: 0,
                b: 128
            },
            hsl: {
                h: 240,
                s: 100,
                l: 25
            },
            name: "Navy"
        },
        Purple: {
            ID: 129,
            hex: "#af00ff",
            rgb: {
                r: 175,
                g: 0,
                b: 255
            },
            hsl: {
                h: 281.176470588235,
                s: 100,
                l: 50
            },
            name: "Purple"
        },
        Teal: {
            ID: 6,
            hex: "#008080",
            rgb: {
                r: 0,
                g: 128,
                b: 128
            },
            hsl: {
                h: 180,
                s: 100,
                l: 25
            },
            name: "Teal"
        },
        Silver: {
            ID: 7,
            hex: "#c0c0c0",
            rgb: {
                r: 192,
                g: 192,
                b: 192
            },
            hsl: {
                h: 0,
                s: 0,
                l: 75
            },
            name: "Silver"
        },
        Grey: {
            ID: 8,
            hex: "#808080",
            rgb: {
                r: 128,
                g: 128,
                b: 128
            },
            hsl: {
                h: 0,
                s: 0,
                l: 50
            },
            name: "Grey"
        },
        Red: {
            ID: 9,
            hex: "#ff0000",
            rgb: {
                r: 255,
                g: 0,
                b: 0
            },
            hsl: {
                h: 0,
                s: 100,
                l: 50
            },
            name: "Red"
        },
        Lime: {
            ID: 10,
            hex: "#00ff00",
            rgb: {
                r: 0,
                g: 255,
                b: 0
            },
            hsl: {
                h: 120,
                s: 100,
                l: 50
            },
            name: "Lime"
        },
        Yellow: {
            ID: 11,
            hex: "#ffff00",
            rgb: {
                r: 255,
                g: 255,
                b: 0
            },
            hsl: {
                h: 60,
                s: 100,
                l: 50
            },
            name: "Yellow"
        },
        Blue: {
            ID: 12,
            hex: "#0000ff",
            rgb: {
                r: 0,
                g: 0,
                b: 255
            },
            hsl: {
                h: 240,
                s: 100,
                l: 50
            },
            name: "Blue"
        },
        Fuchsia: {
            ID: 13,
            hex: "#ff00ff",
            rgb: {
                r: 255,
                g: 0,
                b: 255
            },
            hsl: {
                h: 300,
                s: 100,
                l: 50
            },
            name: "Fuchsia"
        },
        Aqua: {
            ID: 14,
            hex: "#00ffff",
            rgb: {
                r: 0,
                g: 255,
                b: 255
            },
            hsl: {
                h: 180,
                s: 100,
                l: 50
            },
            name: "Aqua"
        },
        White: {
            ID: 15,
            hex: "#ffffff",
            rgb: {
                r: 255,
                g: 255,
                b: 255
            },
            hsl: {
                h: 0,
                s: 0,
                l: 100
            },
            name: "White"
        },
        Grey0: {
            ID: 16,
            hex: "#000000",
            rgb: {
                r: 0,
                g: 0,
                b: 0
            },
            hsl: {
                h: 0,
                s: 0,
                l: 0
            },
            name: "Grey0"
        },
        NavyBlue: {
            ID: 17,
            hex: "#00005f",
            rgb: {
                r: 0,
                g: 0,
                b: 95
            },
            hsl: {
                h: 240,
                s: 100,
                l: 18
            },
            name: "NavyBlue"
        },
        DarkBlue: {
            ID: 18,
            hex: "#000087",
            rgb: {
                r: 0,
                g: 0,
                b: 135
            },
            hsl: {
                h: 240,
                s: 100,
                l: 26
            },
            name: "DarkBlue"
        },
        Blue3: {
            ID: 20,
            hex: "#0000d7",
            rgb: {
                r: 0,
                g: 0,
                b: 215
            },
            hsl: {
                h: 240,
                s: 100,
                l: 42
            },
            name: "Blue3"
        },
        Blue1: {
            ID: 21,
            hex: "#0000ff",
            rgb: {
                r: 0,
                g: 0,
                b: 255
            },
            hsl: {
                h: 240,
                s: 100,
                l: 50
            },
            name: "Blue1"
        },
        DarkGreen: {
            ID: 22,
            hex: "#005f00",
            rgb: {
                r: 0,
                g: 95,
                b: 0
            },
            hsl: {
                h: 120,
                s: 100,
                l: 18
            },
            name: "DarkGreen"
        },
        DeepSkyBlue4: {
            ID: 25,
            hex: "#005faf",
            rgb: {
                r: 0,
                g: 95,
                b: 175
            },
            hsl: {
                h: 207.428571428571,
                s: 100,
                l: 34
            },
            name: "DeepSkyBlue4"
        },
        DodgerBlue3: {
            ID: 26,
            hex: "#005fd7",
            rgb: {
                r: 0,
                g: 95,
                b: 215
            },
            hsl: {
                h: 213.488372093023,
                s: 100,
                l: 42
            },
            name: "DodgerBlue3"
        },
        DodgerBlue2: {
            ID: 27,
            hex: "#005fff",
            rgb: {
                r: 0,
                g: 95,
                b: 255
            },
            hsl: {
                h: 217.647058823529,
                s: 100,
                l: 50
            },
            name: "DodgerBlue2"
        },
        Green4: {
            ID: 28,
            hex: "#008700",
            rgb: {
                r: 0,
                g: 135,
                b: 0
            },
            hsl: {
                h: 120,
                s: 100,
                l: 26
            },
            name: "Green4"
        },
        SpringGreen4: {
            ID: 29,
            hex: "#00875f",
            rgb: {
                r: 0,
                g: 135,
                b: 95
            },
            hsl: {
                h: 162.222222222222,
                s: 100,
                l: 26
            },
            name: "SpringGreen4"
        },
        Turquoise4: {
            ID: 30,
            hex: "#008787",
            rgb: {
                r: 0,
                g: 135,
                b: 135
            },
            hsl: {
                h: 180,
                s: 100,
                l: 26
            },
            name: "Turquoise4"
        },
        DeepSkyBlue3: {
            ID: 32,
            hex: "#0087d7",
            rgb: {
                r: 0,
                g: 135,
                b: 215
            },
            hsl: {
                h: 202.325581395349,
                s: 100,
                l: 42
            },
            name: "DeepSkyBlue3"
        },
        DodgerBlue1: {
            ID: 33,
            hex: "#0087ff",
            rgb: {
                r: 0,
                g: 135,
                b: 255
            },
            hsl: {
                h: 208.235294117647,
                s: 100,
                l: 50
            },
            name: "DodgerBlue1"
        },
        Green3: {
            ID: 40,
            hex: "#00d700",
            rgb: {
                r: 0,
                g: 215,
                b: 0
            },
            hsl: {
                h: 120,
                s: 100,
                l: 42
            },
            name: "Green3"
        },
        SpringGreen3: {
            ID: 41,
            hex: "#00d75f",
            rgb: {
                r: 0,
                g: 215,
                b: 95
            },
            hsl: {
                h: 146.511627906977,
                s: 100,
                l: 42
            },
            name: "SpringGreen3"
        },
        DarkCyan: {
            ID: 36,
            hex: "#00af87",
            rgb: {
                r: 0,
                g: 175,
                b: 135
            },
            hsl: {
                h: 166.285714285714,
                s: 100,
                l: 34
            },
            name: "DarkCyan"
        },
        LightSeaGreen: {
            ID: 37,
            hex: "#00afaf",
            rgb: {
                r: 0,
                g: 175,
                b: 175
            },
            hsl: {
                h: 180,
                s: 100,
                l: 34
            },
            name: "LightSeaGreen"
        },
        DeepSkyBlue2: {
            ID: 38,
            hex: "#00afd7",
            rgb: {
                r: 0,
                g: 175,
                b: 215
            },
            hsl: {
                h: 191.162790697674,
                s: 100,
                l: 42
            },
            name: "DeepSkyBlue2"
        },
        DeepSkyBlue1: {
            ID: 39,
            hex: "#00afff",
            rgb: {
                r: 0,
                g: 175,
                b: 255
            },
            hsl: {
                h: 198.823529411765,
                s: 100,
                l: 50
            },
            name: "DeepSkyBlue1"
        },
        SpringGreen2: {
            ID: 47,
            hex: "#00ff5f",
            rgb: {
                r: 0,
                g: 255,
                b: 95
            },
            hsl: {
                h: 142.352941176471,
                s: 100,
                l: 50
            },
            name: "SpringGreen2"
        },
        Cyan3: {
            ID: 43,
            hex: "#00d7af",
            rgb: {
                r: 0,
                g: 215,
                b: 175
            },
            hsl: {
                h: 168.837209302326,
                s: 100,
                l: 42
            },
            name: "Cyan3"
        },
        DarkTurquoise: {
            ID: 44,
            hex: "#00d7d7",
            rgb: {
                r: 0,
                g: 215,
                b: 215
            },
            hsl: {
                h: 180,
                s: 100,
                l: 42
            },
            name: "DarkTurquoise"
        },
        Turquoise2: {
            ID: 45,
            hex: "#00d7ff",
            rgb: {
                r: 0,
                g: 215,
                b: 255
            },
            hsl: {
                h: 189.411764705882,
                s: 100,
                l: 50
            },
            name: "Turquoise2"
        },
        Green1: {
            ID: 46,
            hex: "#00ff00",
            rgb: {
                r: 0,
                g: 255,
                b: 0
            },
            hsl: {
                h: 120,
                s: 100,
                l: 50
            },
            name: "Green1"
        },
        SpringGreen1: {
            ID: 48,
            hex: "#00ff87",
            rgb: {
                r: 0,
                g: 255,
                b: 135
            },
            hsl: {
                h: 151.764705882353,
                s: 100,
                l: 50
            },
            name: "SpringGreen1"
        },
        MediumSpringGreen: {
            ID: 49,
            hex: "#00ffaf",
            rgb: {
                r: 0,
                g: 255,
                b: 175
            },
            hsl: {
                h: 161.176470588235,
                s: 100,
                l: 50
            },
            name: "MediumSpringGreen"
        },
        Cyan2: {
            ID: 50,
            hex: "#00ffd7",
            rgb: {
                r: 0,
                g: 255,
                b: 215
            },
            hsl: {
                h: 170.588235294118,
                s: 100,
                l: 50
            },
            name: "Cyan2"
        },
        Cyan1: {
            ID: 51,
            hex: "#00ffff",
            rgb: {
                r: 0,
                g: 255,
                b: 255
            },
            hsl: {
                h: 180,
                s: 100,
                l: 50
            },
            name: "Cyan1"
        },
        DarkRed: {
            ID: 88,
            hex: "#870000",
            rgb: {
                r: 135,
                g: 0,
                b: 0
            },
            hsl: {
                h: 0,
                s: 100,
                l: 26
            },
            name: "DarkRed"
        },
        DeepPink4: {
            ID: 125,
            hex: "#af005f",
            rgb: {
                r: 175,
                g: 0,
                b: 95
            },
            hsl: {
                h: 327.428571428571,
                s: 100,
                l: 34
            },
            name: "DeepPink4"
        },
        Purple4: {
            ID: 55,
            hex: "#5f00af",
            rgb: {
                r: 95,
                g: 0,
                b: 175
            },
            hsl: {
                h: 272.571428571429,
                s: 100,
                l: 34
            },
            name: "Purple4"
        },
        Purple3: {
            ID: 56,
            hex: "#5f00d7",
            rgb: {
                r: 95,
                g: 0,
                b: 215
            },
            hsl: {
                h: 266.511627906977,
                s: 100,
                l: 42
            },
            name: "Purple3"
        },
        BlueViolet: {
            ID: 57,
            hex: "#5f00ff",
            rgb: {
                r: 95,
                g: 0,
                b: 255
            },
            hsl: {
                h: 262.352941176471,
                s: 100,
                l: 50
            },
            name: "BlueViolet"
        },
        Orange4: {
            ID: 94,
            hex: "#875f00",
            rgb: {
                r: 135,
                g: 95,
                b: 0
            },
            hsl: {
                h: 42.2222222222222,
                s: 100,
                l: 26
            },
            name: "Orange4"
        },
        Grey37: {
            ID: 59,
            hex: "#5f5f5f",
            rgb: {
                r: 95,
                g: 95,
                b: 95
            },
            hsl: {
                h: 0,
                s: 0,
                l: 37
            },
            name: "Grey37"
        },
        MediumPurple4: {
            ID: 60,
            hex: "#5f5f87",
            rgb: {
                r: 95,
                g: 95,
                b: 135
            },
            hsl: {
                h: 240,
                s: 17,
                l: 45
            },
            name: "MediumPurple4"
        },
        SlateBlue3: {
            ID: 62,
            hex: "#5f5fd7",
            rgb: {
                r: 95,
                g: 95,
                b: 215
            },
            hsl: {
                h: 240,
                s: 60,
                l: 60
            },
            name: "SlateBlue3"
        },
        RoyalBlue1: {
            ID: 63,
            hex: "#5f5fff",
            rgb: {
                r: 95,
                g: 95,
                b: 255
            },
            hsl: {
                h: 240,
                s: 100,
                l: 68
            },
            name: "RoyalBlue1"
        },
        Chartreuse4: {
            ID: 64,
            hex: "#5f8700",
            rgb: {
                r: 95,
                g: 135,
                b: 0
            },
            hsl: {
                h: 77.7777777777778,
                s: 100,
                l: 26
            },
            name: "Chartreuse4"
        },
        DarkSeaGreen4: {
            ID: 71,
            hex: "#5faf5f",
            rgb: {
                r: 95,
                g: 175,
                b: 95
            },
            hsl: {
                h: 120,
                s: 33,
                l: 52
            },
            name: "DarkSeaGreen4"
        },
        PaleTurquoise4: {
            ID: 66,
            hex: "#5f8787",
            rgb: {
                r: 95,
                g: 135,
                b: 135
            },
            hsl: {
                h: 180,
                s: 17,
                l: 45
            },
            name: "PaleTurquoise4"
        },
        SteelBlue: {
            ID: 67,
            hex: "#5f87af",
            rgb: {
                r: 95,
                g: 135,
                b: 175
            },
            hsl: {
                h: 210,
                s: 33,
                l: 52
            },
            name: "SteelBlue"
        },
        SteelBlue3: {
            ID: 68,
            hex: "#5f87d7",
            rgb: {
                r: 95,
                g: 135,
                b: 215
            },
            hsl: {
                h: 220,
                s: 60,
                l: 60
            },
            name: "SteelBlue3"
        },
        CornflowerBlue: {
            ID: 69,
            hex: "#5f87ff",
            rgb: {
                r: 95,
                g: 135,
                b: 255
            },
            hsl: {
                h: 225,
                s: 100,
                l: 68
            },
            name: "CornflowerBlue"
        },
        Chartreuse3: {
            ID: 76,
            hex: "#5fd700",
            rgb: {
                r: 95,
                g: 215,
                b: 0
            },
            hsl: {
                h: 93.4883720930233,
                s: 100,
                l: 42
            },
            name: "Chartreuse3"
        },
        CadetBlue: {
            ID: 73,
            hex: "#5fafaf",
            rgb: {
                r: 95,
                g: 175,
                b: 175
            },
            hsl: {
                h: 180,
                s: 33,
                l: 52
            },
            name: "CadetBlue"
        },
        SkyBlue3: {
            ID: 74,
            hex: "#5fafd7",
            rgb: {
                r: 95,
                g: 175,
                b: 215
            },
            hsl: {
                h: 200,
                s: 60,
                l: 60
            },
            name: "SkyBlue3"
        },
        SteelBlue1: {
            ID: 81,
            hex: "#5fd7ff",
            rgb: {
                r: 95,
                g: 215,
                b: 255
            },
            hsl: {
                h: 195,
                s: 100,
                l: 68
            },
            name: "SteelBlue1"
        },
        PaleGreen3: {
            ID: 114,
            hex: "#87d787",
            rgb: {
                r: 135,
                g: 215,
                b: 135
            },
            hsl: {
                h: 120,
                s: 50,
                l: 68
            },
            name: "PaleGreen3"
        },
        SeaGreen3: {
            ID: 78,
            hex: "#5fd787",
            rgb: {
                r: 95,
                g: 215,
                b: 135
            },
            hsl: {
                h: 140,
                s: 60,
                l: 60
            },
            name: "SeaGreen3"
        },
        Aquamarine3: {
            ID: 79,
            hex: "#5fd7af",
            rgb: {
                r: 95,
                g: 215,
                b: 175
            },
            hsl: {
                h: 160,
                s: 60,
                l: 60
            },
            name: "Aquamarine3"
        },
        MediumTurquoise: {
            ID: 80,
            hex: "#5fd7d7",
            rgb: {
                r: 95,
                g: 215,
                b: 215
            },
            hsl: {
                h: 180,
                s: 60,
                l: 60
            },
            name: "MediumTurquoise"
        },
        Chartreuse2: {
            ID: 112,
            hex: "#87d700",
            rgb: {
                r: 135,
                g: 215,
                b: 0
            },
            hsl: {
                h: 82.3255813953488,
                s: 100,
                l: 42
            },
            name: "Chartreuse2"
        },
        SeaGreen2: {
            ID: 83,
            hex: "#5fff5f",
            rgb: {
                r: 95,
                g: 255,
                b: 95
            },
            hsl: {
                h: 120,
                s: 100,
                l: 68
            },
            name: "SeaGreen2"
        },
        SeaGreen1: {
            ID: 85,
            hex: "#5fffaf",
            rgb: {
                r: 95,
                g: 255,
                b: 175
            },
            hsl: {
                h: 150,
                s: 100,
                l: 68
            },
            name: "SeaGreen1"
        },
        Aquamarine1: {
            ID: 122,
            hex: "#87ffd7",
            rgb: {
                r: 135,
                g: 255,
                b: 215
            },
            hsl: {
                h: 160,
                s: 100,
                l: 76
            },
            name: "Aquamarine1"
        },
        DarkSlateGray2: {
            ID: 87,
            hex: "#5fffff",
            rgb: {
                r: 95,
                g: 255,
                b: 255
            },
            hsl: {
                h: 180,
                s: 100,
                l: 68
            },
            name: "DarkSlateGray2"
        },
        DarkMagenta: {
            ID: 91,
            hex: "#8700af",
            rgb: {
                r: 135,
                g: 0,
                b: 175
            },
            hsl: {
                h: 286.285714285714,
                s: 100,
                l: 34
            },
            name: "DarkMagenta"
        },
        DarkViolet: {
            ID: 128,
            hex: "#af00d7",
            rgb: {
                r: 175,
                g: 0,
                b: 215
            },
            hsl: {
                h: 288.837209302326,
                s: 100,
                l: 42
            },
            name: "DarkViolet"
        },
        LightPink4: {
            ID: 95,
            hex: "#875f5f",
            rgb: {
                r: 135,
                g: 95,
                b: 95
            },
            hsl: {
                h: 0,
                s: 17,
                l: 45
            },
            name: "LightPink4"
        },
        Plum4: {
            ID: 96,
            hex: "#875f87",
            rgb: {
                r: 135,
                g: 95,
                b: 135
            },
            hsl: {
                h: 300,
                s: 17,
                l: 45
            },
            name: "Plum4"
        },
        MediumPurple3: {
            ID: 98,
            hex: "#875fd7",
            rgb: {
                r: 135,
                g: 95,
                b: 215
            },
            hsl: {
                h: 260,
                s: 60,
                l: 60
            },
            name: "MediumPurple3"
        },
        SlateBlue1: {
            ID: 99,
            hex: "#875fff",
            rgb: {
                r: 135,
                g: 95,
                b: 255
            },
            hsl: {
                h: 255,
                s: 100,
                l: 68
            },
            name: "SlateBlue1"
        },
        Yellow4: {
            ID: 106,
            hex: "#87af00",
            rgb: {
                r: 135,
                g: 175,
                b: 0
            },
            hsl: {
                h: 73.7142857142857,
                s: 100,
                l: 34
            },
            name: "Yellow4"
        },
        Wheat4: {
            ID: 101,
            hex: "#87875f",
            rgb: {
                r: 135,
                g: 135,
                b: 95
            },
            hsl: {
                h: 60,
                s: 17,
                l: 45
            },
            name: "Wheat4"
        },
        Grey53: {
            ID: 102,
            hex: "#878787",
            rgb: {
                r: 135,
                g: 135,
                b: 135
            },
            hsl: {
                h: 0,
                s: 0,
                l: 52
            },
            name: "Grey53"
        },
        LightSlateGrey: {
            ID: 103,
            hex: "#8787af",
            rgb: {
                r: 135,
                g: 135,
                b: 175
            },
            hsl: {
                h: 240,
                s: 20,
                l: 60
            },
            name: "LightSlateGrey"
        },
        MediumPurple: {
            ID: 104,
            hex: "#8787d7",
            rgb: {
                r: 135,
                g: 135,
                b: 215
            },
            hsl: {
                h: 240,
                s: 50,
                l: 68
            },
            name: "MediumPurple"
        },
        LightSlateBlue: {
            ID: 105,
            hex: "#8787ff",
            rgb: {
                r: 135,
                g: 135,
                b: 255
            },
            hsl: {
                h: 240,
                s: 100,
                l: 76
            },
            name: "LightSlateBlue"
        },
        DarkOliveGreen3: {
            ID: 149,
            hex: "#afd75f",
            rgb: {
                r: 175,
                g: 215,
                b: 95
            },
            hsl: {
                h: 80,
                s: 60,
                l: 60
            },
            name: "DarkOliveGreen3"
        },
        DarkSeaGreen: {
            ID: 108,
            hex: "#87af87",
            rgb: {
                r: 135,
                g: 175,
                b: 135
            },
            hsl: {
                h: 120,
                s: 20,
                l: 60
            },
            name: "DarkSeaGreen"
        },
        LightSkyBlue3: {
            ID: 110,
            hex: "#87afd7",
            rgb: {
                r: 135,
                g: 175,
                b: 215
            },
            hsl: {
                h: 210,
                s: 50,
                l: 68
            },
            name: "LightSkyBlue3"
        },
        SkyBlue2: {
            ID: 111,
            hex: "#87afff",
            rgb: {
                r: 135,
                g: 175,
                b: 255
            },
            hsl: {
                h: 220,
                s: 100,
                l: 76
            },
            name: "SkyBlue2"
        },
        DarkSeaGreen3: {
            ID: 150,
            hex: "#afd787",
            rgb: {
                r: 175,
                g: 215,
                b: 135
            },
            hsl: {
                h: 90,
                s: 50,
                l: 68
            },
            name: "DarkSeaGreen3"
        },
        DarkSlateGray3: {
            ID: 116,
            hex: "#87d7d7",
            rgb: {
                r: 135,
                g: 215,
                b: 215
            },
            hsl: {
                h: 180,
                s: 50,
                l: 68
            },
            name: "DarkSlateGray3"
        },
        SkyBlue1: {
            ID: 117,
            hex: "#87d7ff",
            rgb: {
                r: 135,
                g: 215,
                b: 255
            },
            hsl: {
                h: 200,
                s: 100,
                l: 76
            },
            name: "SkyBlue1"
        },
        Chartreuse1: {
            ID: 118,
            hex: "#87ff00",
            rgb: {
                r: 135,
                g: 255,
                b: 0
            },
            hsl: {
                h: 88.2352941176471,
                s: 100,
                l: 50
            },
            name: "Chartreuse1"
        },
        LightGreen: {
            ID: 120,
            hex: "#87ff87",
            rgb: {
                r: 135,
                g: 255,
                b: 135
            },
            hsl: {
                h: 120,
                s: 100,
                l: 76
            },
            name: "LightGreen"
        },
        PaleGreen1: {
            ID: 156,
            hex: "#afff87",
            rgb: {
                r: 175,
                g: 255,
                b: 135
            },
            hsl: {
                h: 100,
                s: 100,
                l: 76
            },
            name: "PaleGreen1"
        },
        DarkSlateGray1: {
            ID: 123,
            hex: "#87ffff",
            rgb: {
                r: 135,
                g: 255,
                b: 255
            },
            hsl: {
                h: 180,
                s: 100,
                l: 76
            },
            name: "DarkSlateGray1"
        },
        Red3: {
            ID: 160,
            hex: "#d70000",
            rgb: {
                r: 215,
                g: 0,
                b: 0
            },
            hsl: {
                h: 0,
                s: 100,
                l: 42
            },
            name: "Red3"
        },
        MediumVioletRed: {
            ID: 126,
            hex: "#af0087",
            rgb: {
                r: 175,
                g: 0,
                b: 135
            },
            hsl: {
                h: 313.714285714286,
                s: 100,
                l: 34
            },
            name: "MediumVioletRed"
        },
        Magenta3: {
            ID: 164,
            hex: "#d700d7",
            rgb: {
                r: 215,
                g: 0,
                b: 215
            },
            hsl: {
                h: 300,
                s: 100,
                l: 42
            },
            name: "Magenta3"
        },
        DarkOrange3: {
            ID: 166,
            hex: "#d75f00",
            rgb: {
                r: 215,
                g: 95,
                b: 0
            },
            hsl: {
                h: 26.5116279069767,
                s: 100,
                l: 42
            },
            name: "DarkOrange3"
        },
        IndianRed: {
            ID: 167,
            hex: "#d75f5f",
            rgb: {
                r: 215,
                g: 95,
                b: 95
            },
            hsl: {
                h: 0,
                s: 60,
                l: 60
            },
            name: "IndianRed"
        },
        HotPink3: {
            ID: 168,
            hex: "#d75f87",
            rgb: {
                r: 215,
                g: 95,
                b: 135
            },
            hsl: {
                h: 340,
                s: 60,
                l: 60
            },
            name: "HotPink3"
        },
        MediumOrchid3: {
            ID: 133,
            hex: "#af5faf",
            rgb: {
                r: 175,
                g: 95,
                b: 175
            },
            hsl: {
                h: 300,
                s: 33,
                l: 52
            },
            name: "MediumOrchid3"
        },
        MediumOrchid: {
            ID: 134,
            hex: "#af5fd7",
            rgb: {
                r: 175,
                g: 95,
                b: 215
            },
            hsl: {
                h: 280,
                s: 60,
                l: 60
            },
            name: "MediumOrchid"
        },
        MediumPurple2: {
            ID: 140,
            hex: "#af87d7",
            rgb: {
                r: 175,
                g: 135,
                b: 215
            },
            hsl: {
                h: 270,
                s: 50,
                l: 68
            },
            name: "MediumPurple2"
        },
        DarkGoldenrod: {
            ID: 136,
            hex: "#af8700",
            rgb: {
                r: 175,
                g: 135,
                b: 0
            },
            hsl: {
                h: 46.2857142857143,
                s: 100,
                l: 34
            },
            name: "DarkGoldenrod"
        },
        LightSalmon3: {
            ID: 173,
            hex: "#d7875f",
            rgb: {
                r: 215,
                g: 135,
                b: 95
            },
            hsl: {
                h: 20,
                s: 60,
                l: 60
            },
            name: "LightSalmon3"
        },
        RosyBrown: {
            ID: 138,
            hex: "#af8787",
            rgb: {
                r: 175,
                g: 135,
                b: 135
            },
            hsl: {
                h: 0,
                s: 20,
                l: 60
            },
            name: "RosyBrown"
        },
        Grey63: {
            ID: 139,
            hex: "#af87af",
            rgb: {
                r: 175,
                g: 135,
                b: 175
            },
            hsl: {
                h: 300,
                s: 20,
                l: 60
            },
            name: "Grey63"
        },
        MediumPurple1: {
            ID: 141,
            hex: "#af87ff",
            rgb: {
                r: 175,
                g: 135,
                b: 255
            },
            hsl: {
                h: 260,
                s: 100,
                l: 76
            },
            name: "MediumPurple1"
        },
        Gold3: {
            ID: 178,
            hex: "#d7af00",
            rgb: {
                r: 215,
                g: 175,
                b: 0
            },
            hsl: {
                h: 48.8372093023256,
                s: 100,
                l: 42
            },
            name: "Gold3"
        },
        DarkKhaki: {
            ID: 143,
            hex: "#afaf5f",
            rgb: {
                r: 175,
                g: 175,
                b: 95
            },
            hsl: {
                h: 60,
                s: 33,
                l: 52
            },
            name: "DarkKhaki"
        },
        NavajoWhite3: {
            ID: 144,
            hex: "#afaf87",
            rgb: {
                r: 175,
                g: 175,
                b: 135
            },
            hsl: {
                h: 60,
                s: 20,
                l: 60
            },
            name: "NavajoWhite3"
        },
        Grey69: {
            ID: 145,
            hex: "#afafaf",
            rgb: {
                r: 175,
                g: 175,
                b: 175
            },
            hsl: {
                h: 0,
                s: 0,
                l: 68
            },
            name: "Grey69"
        },
        LightSteelBlue3: {
            ID: 146,
            hex: "#afafd7",
            rgb: {
                r: 175,
                g: 175,
                b: 215
            },
            hsl: {
                h: 240,
                s: 33,
                l: 76
            },
            name: "LightSteelBlue3"
        },
        LightSteelBlue: {
            ID: 147,
            hex: "#afafff",
            rgb: {
                r: 175,
                g: 175,
                b: 255
            },
            hsl: {
                h: 240,
                s: 100,
                l: 84
            },
            name: "LightSteelBlue"
        },
        Yellow3: {
            ID: 184,
            hex: "#d7d700",
            rgb: {
                r: 215,
                g: 215,
                b: 0
            },
            hsl: {
                h: 60,
                s: 100,
                l: 42
            },
            name: "Yellow3"
        },
        DarkSeaGreen2: {
            ID: 157,
            hex: "#afffaf",
            rgb: {
                r: 175,
                g: 255,
                b: 175
            },
            hsl: {
                h: 120,
                s: 100,
                l: 84
            },
            name: "DarkSeaGreen2"
        },
        LightCyan3: {
            ID: 152,
            hex: "#afd7d7",
            rgb: {
                r: 175,
                g: 215,
                b: 215
            },
            hsl: {
                h: 180,
                s: 33,
                l: 76
            },
            name: "LightCyan3"
        },
        LightSkyBlue1: {
            ID: 153,
            hex: "#afd7ff",
            rgb: {
                r: 175,
                g: 215,
                b: 255
            },
            hsl: {
                h: 210,
                s: 100,
                l: 84
            },
            name: "LightSkyBlue1"
        },
        GreenYellow: {
            ID: 154,
            hex: "#afff00",
            rgb: {
                r: 175,
                g: 255,
                b: 0
            },
            hsl: {
                h: 78.8235294117647,
                s: 100,
                l: 50
            },
            name: "GreenYellow"
        },
        DarkOliveGreen2: {
            ID: 155,
            hex: "#afff5f",
            rgb: {
                r: 175,
                g: 255,
                b: 95
            },
            hsl: {
                h: 90,
                s: 100,
                l: 68
            },
            name: "DarkOliveGreen2"
        },
        DarkSeaGreen1: {
            ID: 193,
            hex: "#d7ffaf",
            rgb: {
                r: 215,
                g: 255,
                b: 175
            },
            hsl: {
                h: 90,
                s: 100,
                l: 84
            },
            name: "DarkSeaGreen1"
        },
        PaleTurquoise1: {
            ID: 159,
            hex: "#afffff",
            rgb: {
                r: 175,
                g: 255,
                b: 255
            },
            hsl: {
                h: 180,
                s: 100,
                l: 84
            },
            name: "PaleTurquoise1"
        },
        DeepPink3: {
            ID: 162,
            hex: "#d70087",
            rgb: {
                r: 215,
                g: 0,
                b: 135
            },
            hsl: {
                h: 322.325581395349,
                s: 100,
                l: 42
            },
            name: "DeepPink3"
        },
        Magenta2: {
            ID: 200,
            hex: "#ff00d7",
            rgb: {
                r: 255,
                g: 0,
                b: 215
            },
            hsl: {
                h: 309.411764705882,
                s: 100,
                l: 50
            },
            name: "Magenta2"
        },
        HotPink2: {
            ID: 169,
            hex: "#d75faf",
            rgb: {
                r: 215,
                g: 95,
                b: 175
            },
            hsl: {
                h: 320,
                s: 60,
                l: 60
            },
            name: "HotPink2"
        },
        Orchid: {
            ID: 170,
            hex: "#d75fd7",
            rgb: {
                r: 215,
                g: 95,
                b: 215
            },
            hsl: {
                h: 300,
                s: 60,
                l: 60
            },
            name: "Orchid"
        },
        MediumOrchid1: {
            ID: 207,
            hex: "#ff5fff",
            rgb: {
                r: 255,
                g: 95,
                b: 255
            },
            hsl: {
                h: 300,
                s: 100,
                l: 68
            },
            name: "MediumOrchid1"
        },
        Orange3: {
            ID: 172,
            hex: "#d78700",
            rgb: {
                r: 215,
                g: 135,
                b: 0
            },
            hsl: {
                h: 37.6744186046512,
                s: 100,
                l: 42
            },
            name: "Orange3"
        },
        LightPink3: {
            ID: 174,
            hex: "#d78787",
            rgb: {
                r: 215,
                g: 135,
                b: 135
            },
            hsl: {
                h: 0,
                s: 50,
                l: 68
            },
            name: "LightPink3"
        },
        Pink3: {
            ID: 175,
            hex: "#d787af",
            rgb: {
                r: 215,
                g: 135,
                b: 175
            },
            hsl: {
                h: 330,
                s: 50,
                l: 68
            },
            name: "Pink3"
        },
        Plum3: {
            ID: 176,
            hex: "#d787d7",
            rgb: {
                r: 215,
                g: 135,
                b: 215
            },
            hsl: {
                h: 300,
                s: 50,
                l: 68
            },
            name: "Plum3"
        },
        Violet: {
            ID: 177,
            hex: "#d787ff",
            rgb: {
                r: 215,
                g: 135,
                b: 255
            },
            hsl: {
                h: 280,
                s: 100,
                l: 76
            },
            name: "Violet"
        },
        LightGoldenrod3: {
            ID: 179,
            hex: "#d7af5f",
            rgb: {
                r: 215,
                g: 175,
                b: 95
            },
            hsl: {
                h: 40,
                s: 60,
                l: 60
            },
            name: "LightGoldenrod3"
        },
        Tan: {
            ID: 180,
            hex: "#d7af87",
            rgb: {
                r: 215,
                g: 175,
                b: 135
            },
            hsl: {
                h: 30,
                s: 50,
                l: 68
            },
            name: "Tan"
        },
        MistyRose3: {
            ID: 181,
            hex: "#d7afaf",
            rgb: {
                r: 215,
                g: 175,
                b: 175
            },
            hsl: {
                h: 0,
                s: 33,
                l: 76
            },
            name: "MistyRose3"
        },
        Thistle3: {
            ID: 182,
            hex: "#d7afd7",
            rgb: {
                r: 215,
                g: 175,
                b: 215
            },
            hsl: {
                h: 300,
                s: 33,
                l: 76
            },
            name: "Thistle3"
        },
        Plum2: {
            ID: 183,
            hex: "#d7afff",
            rgb: {
                r: 215,
                g: 175,
                b: 255
            },
            hsl: {
                h: 270,
                s: 100,
                l: 84
            },
            name: "Plum2"
        },
        Khaki3: {
            ID: 185,
            hex: "#d7d75f",
            rgb: {
                r: 215,
                g: 215,
                b: 95
            },
            hsl: {
                h: 60,
                s: 60,
                l: 60
            },
            name: "Khaki3"
        },
        LightGoldenrod2: {
            ID: 222,
            hex: "#ffd787",
            rgb: {
                r: 255,
                g: 215,
                b: 135
            },
            hsl: {
                h: 40,
                s: 100,
                l: 76
            },
            name: "LightGoldenrod2"
        },
        LightYellow3: {
            ID: 187,
            hex: "#d7d7af",
            rgb: {
                r: 215,
                g: 215,
                b: 175
            },
            hsl: {
                h: 60,
                s: 33,
                l: 76
            },
            name: "LightYellow3"
        },
        Grey84: {
            ID: 188,
            hex: "#d7d7d7",
            rgb: {
                r: 215,
                g: 215,
                b: 215
            },
            hsl: {
                h: 0,
                s: 0,
                l: 84
            },
            name: "Grey84"
        },
        LightSteelBlue1: {
            ID: 189,
            hex: "#d7d7ff",
            rgb: {
                r: 215,
                g: 215,
                b: 255
            },
            hsl: {
                h: 240,
                s: 100,
                l: 92
            },
            name: "LightSteelBlue1"
        },
        Yellow2: {
            ID: 190,
            hex: "#d7ff00",
            rgb: {
                r: 215,
                g: 255,
                b: 0
            },
            hsl: {
                h: 69.4117647058823,
                s: 100,
                l: 50
            },
            name: "Yellow2"
        },
        DarkOliveGreen1: {
            ID: 192,
            hex: "#d7ff87",
            rgb: {
                r: 215,
                g: 255,
                b: 135
            },
            hsl: {
                h: 80,
                s: 100,
                l: 76
            },
            name: "DarkOliveGreen1"
        },
        Honeydew2: {
            ID: 194,
            hex: "#d7ffd7",
            rgb: {
                r: 215,
                g: 255,
                b: 215
            },
            hsl: {
                h: 120,
                s: 100,
                l: 92
            },
            name: "Honeydew2"
        },
        LightCyan1: {
            ID: 195,
            hex: "#d7ffff",
            rgb: {
                r: 215,
                g: 255,
                b: 255
            },
            hsl: {
                h: 180,
                s: 100,
                l: 92
            },
            name: "LightCyan1"
        },
        Red1: {
            ID: 196,
            hex: "#ff0000",
            rgb: {
                r: 255,
                g: 0,
                b: 0
            },
            hsl: {
                h: 0,
                s: 100,
                l: 50
            },
            name: "Red1"
        },
        DeepPink2: {
            ID: 197,
            hex: "#ff005f",
            rgb: {
                r: 255,
                g: 0,
                b: 95
            },
            hsl: {
                h: 337.647058823529,
                s: 100,
                l: 50
            },
            name: "DeepPink2"
        },
        DeepPink1: {
            ID: 199,
            hex: "#ff00af",
            rgb: {
                r: 255,
                g: 0,
                b: 175
            },
            hsl: {
                h: 318.823529411765,
                s: 100,
                l: 50
            },
            name: "DeepPink1"
        },
        Magenta1: {
            ID: 201,
            hex: "#ff00ff",
            rgb: {
                r: 255,
                g: 0,
                b: 255
            },
            hsl: {
                h: 300,
                s: 100,
                l: 50
            },
            name: "Magenta1"
        },
        OrangeRed1: {
            ID: 202,
            hex: "#ff5f00",
            rgb: {
                r: 255,
                g: 95,
                b: 0
            },
            hsl: {
                h: 22.3529411764706,
                s: 100,
                l: 50
            },
            name: "OrangeRed1"
        },
        IndianRed1: {
            ID: 204,
            hex: "#ff5f87",
            rgb: {
                r: 255,
                g: 95,
                b: 135
            },
            hsl: {
                h: 345,
                s: 100,
                l: 68
            },
            name: "IndianRed1"
        },
        HotPink: {
            ID: 206,
            hex: "#ff5fd7",
            rgb: {
                r: 255,
                g: 95,
                b: 215
            },
            hsl: {
                h: 315,
                s: 100,
                l: 68
            },
            name: "HotPink"
        },
        DarkOrange: {
            ID: 208,
            hex: "#ff8700",
            rgb: {
                r: 255,
                g: 135,
                b: 0
            },
            hsl: {
                h: 31.7647058823529,
                s: 100,
                l: 50
            },
            name: "DarkOrange"
        },
        Salmon1: {
            ID: 209,
            hex: "#ff875f",
            rgb: {
                r: 255,
                g: 135,
                b: 95
            },
            hsl: {
                h: 15,
                s: 100,
                l: 68
            },
            name: "Salmon1"
        },
        LightCoral: {
            ID: 210,
            hex: "#ff8787",
            rgb: {
                r: 255,
                g: 135,
                b: 135
            },
            hsl: {
                h: 0,
                s: 100,
                l: 76
            },
            name: "LightCoral"
        },
        PaleVioletRed1: {
            ID: 211,
            hex: "#ff87af",
            rgb: {
                r: 255,
                g: 135,
                b: 175
            },
            hsl: {
                h: 340,
                s: 100,
                l: 76
            },
            name: "PaleVioletRed1"
        },
        Orchid2: {
            ID: 212,
            hex: "#ff87d7",
            rgb: {
                r: 255,
                g: 135,
                b: 215
            },
            hsl: {
                h: 320,
                s: 100,
                l: 76
            },
            name: "Orchid2"
        },
        Orchid1: {
            ID: 213,
            hex: "#ff87ff",
            rgb: {
                r: 255,
                g: 135,
                b: 255
            },
            hsl: {
                h: 300,
                s: 100,
                l: 76
            },
            name: "Orchid1"
        },
        Orange1: {
            ID: 214,
            hex: "#ffaf00",
            rgb: {
                r: 255,
                g: 175,
                b: 0
            },
            hsl: {
                h: 41.1764705882353,
                s: 100,
                l: 50
            },
            name: "Orange1"
        },
        SandyBrown: {
            ID: 215,
            hex: "#ffaf5f",
            rgb: {
                r: 255,
                g: 175,
                b: 95
            },
            hsl: {
                h: 30,
                s: 100,
                l: 68
            },
            name: "SandyBrown"
        },
        LightSalmon1: {
            ID: 216,
            hex: "#ffaf87",
            rgb: {
                r: 255,
                g: 175,
                b: 135
            },
            hsl: {
                h: 20,
                s: 100,
                l: 76
            },
            name: "LightSalmon1"
        },
        LightPink1: {
            ID: 217,
            hex: "#ffafaf",
            rgb: {
                r: 255,
                g: 175,
                b: 175
            },
            hsl: {
                h: 0,
                s: 100,
                l: 84
            },
            name: "LightPink1"
        },
        Pink1: {
            ID: 218,
            hex: "#ffafd7",
            rgb: {
                r: 255,
                g: 175,
                b: 215
            },
            hsl: {
                h: 330,
                s: 100,
                l: 84
            },
            name: "Pink1"
        },
        Plum1: {
            ID: 219,
            hex: "#ffafff",
            rgb: {
                r: 255,
                g: 175,
                b: 255
            },
            hsl: {
                h: 300,
                s: 100,
                l: 84
            },
            name: "Plum1"
        },
        Gold1: {
            ID: 220,
            hex: "#ffd700",
            rgb: {
                r: 255,
                g: 215,
                b: 0
            },
            hsl: {
                h: 50.5882352941176,
                s: 100,
                l: 50
            },
            name: "Gold1"
        },
        NavajoWhite1: {
            ID: 223,
            hex: "#ffd7af",
            rgb: {
                r: 255,
                g: 215,
                b: 175
            },
            hsl: {
                h: 30,
                s: 100,
                l: 84
            },
            name: "NavajoWhite1"
        },
        MistyRose1: {
            ID: 224,
            hex: "#ffd7d7",
            rgb: {
                r: 255,
                g: 215,
                b: 215
            },
            hsl: {
                h: 0,
                s: 100,
                l: 92
            },
            name: "MistyRose1"
        },
        Thistle1: {
            ID: 225,
            hex: "#ffd7ff",
            rgb: {
                r: 255,
                g: 215,
                b: 255
            },
            hsl: {
                h: 300,
                s: 100,
                l: 92
            },
            name: "Thistle1"
        },
        Yellow1: {
            ID: 226,
            hex: "#ffff00",
            rgb: {
                r: 255,
                g: 255,
                b: 0
            },
            hsl: {
                h: 60,
                s: 100,
                l: 50
            },
            name: "Yellow1"
        },
        LightGoldenrod1: {
            ID: 227,
            hex: "#ffff5f",
            rgb: {
                r: 255,
                g: 255,
                b: 95
            },
            hsl: {
                h: 60,
                s: 100,
                l: 68
            },
            name: "LightGoldenrod1"
        },
        Khaki1: {
            ID: 228,
            hex: "#ffff87",
            rgb: {
                r: 255,
                g: 255,
                b: 135
            },
            hsl: {
                h: 60,
                s: 100,
                l: 76
            },
            name: "Khaki1"
        },
        Wheat1: {
            ID: 229,
            hex: "#ffffaf",
            rgb: {
                r: 255,
                g: 255,
                b: 175
            },
            hsl: {
                h: 60,
                s: 100,
                l: 84
            },
            name: "Wheat1"
        },
        Cornsilk1: {
            ID: 230,
            hex: "#ffffd7",
            rgb: {
                r: 255,
                g: 255,
                b: 215
            },
            hsl: {
                h: 60,
                s: 100,
                l: 92
            },
            name: "Cornsilk1"
        },
        Grey100: {
            ID: 231,
            hex: "#ffffff",
            rgb: {
                r: 255,
                g: 255,
                b: 255
            },
            hsl: {
                h: 0,
                s: 0,
                l: 100
            },
            name: "Grey100"
        },
        Grey3: {
            ID: 232,
            hex: "#080808",
            rgb: {
                r: 8,
                g: 8,
                b: 8
            },
            hsl: {
                h: 0,
                s: 0,
                l: 3
            },
            name: "Grey3"
        },
        Grey7: {
            ID: 233,
            hex: "#121212",
            rgb: {
                r: 18,
                g: 18,
                b: 18
            },
            hsl: {
                h: 0,
                s: 0,
                l: 7
            },
            name: "Grey7"
        },
        Grey11: {
            ID: 234,
            hex: "#1c1c1c",
            rgb: {
                r: 28,
                g: 28,
                b: 28
            },
            hsl: {
                h: 0,
                s: 0,
                l: 10
            },
            name: "Grey11"
        },
        Grey15: {
            ID: 235,
            hex: "#262626",
            rgb: {
                r: 38,
                g: 38,
                b: 38
            },
            hsl: {
                h: 0,
                s: 0,
                l: 14
            },
            name: "Grey15"
        },
        Grey19: {
            ID: 236,
            hex: "#303030",
            rgb: {
                r: 48,
                g: 48,
                b: 48
            },
            hsl: {
                h: 0,
                s: 0,
                l: 18
            },
            name: "Grey19"
        },
        Grey23: {
            ID: 237,
            hex: "#3a3a3a",
            rgb: {
                r: 58,
                g: 58,
                b: 58
            },
            hsl: {
                h: 0,
                s: 0,
                l: 22
            },
            name: "Grey23"
        },
        Grey27: {
            ID: 238,
            hex: "#444444",
            rgb: {
                r: 68,
                g: 68,
                b: 68
            },
            hsl: {
                h: 0,
                s: 0,
                l: 26
            },
            name: "Grey27"
        },
        Grey30: {
            ID: 239,
            hex: "#4e4e4e",
            rgb: {
                r: 78,
                g: 78,
                b: 78
            },
            hsl: {
                h: 0,
                s: 0,
                l: 30
            },
            name: "Grey30"
        },
        Grey35: {
            ID: 240,
            hex: "#585858",
            rgb: {
                r: 88,
                g: 88,
                b: 88
            },
            hsl: {
                h: 0,
                s: 0,
                l: 34
            },
            name: "Grey35"
        },
        Grey39: {
            ID: 241,
            hex: "#626262",
            rgb: {
                r: 98,
                g: 98,
                b: 98
            },
            hsl: {
                h: 0,
                s: 0,
                l: 37
            },
            name: "Grey39"
        },
        Grey42: {
            ID: 242,
            hex: "#6c6c6c",
            rgb: {
                r: 108,
                g: 108,
                b: 108
            },
            hsl: {
                h: 0,
                s: 0,
                l: 40
            },
            name: "Grey42"
        },
        Grey46: {
            ID: 243,
            hex: "#767676",
            rgb: {
                r: 118,
                g: 118,
                b: 118
            },
            hsl: {
                h: 0,
                s: 0,
                l: 46
            },
            name: "Grey46"
        },
        Grey50: {
            ID: 244,
            hex: "#808080",
            rgb: {
                r: 128,
                g: 128,
                b: 128
            },
            hsl: {
                h: 0,
                s: 0,
                l: 50
            },
            name: "Grey50"
        },
        Grey54: {
            ID: 245,
            hex: "#8a8a8a",
            rgb: {
                r: 138,
                g: 138,
                b: 138
            },
            hsl: {
                h: 0,
                s: 0,
                l: 54
            },
            name: "Grey54"
        },
        Grey58: {
            ID: 246,
            hex: "#949494",
            rgb: {
                r: 148,
                g: 148,
                b: 148
            },
            hsl: {
                h: 0,
                s: 0,
                l: 58
            },
            name: "Grey58"
        },
        Grey62: {
            ID: 247,
            hex: "#9e9e9e",
            rgb: {
                r: 158,
                g: 158,
                b: 158
            },
            hsl: {
                h: 0,
                s: 0,
                l: 61
            },
            name: "Grey62"
        },
        Grey66: {
            ID: 248,
            hex: "#a8a8a8",
            rgb: {
                r: 168,
                g: 168,
                b: 168
            },
            hsl: {
                h: 0,
                s: 0,
                l: 65
            },
            name: "Grey66"
        },
        Grey70: {
            ID: 249,
            hex: "#b2b2b2",
            rgb: {
                r: 178,
                g: 178,
                b: 178
            },
            hsl: {
                h: 0,
                s: 0,
                l: 69
            },
            name: "Grey70"
        },
        Grey74: {
            ID: 250,
            hex: "#bcbcbc",
            rgb: {
                r: 188,
                g: 188,
                b: 188
            },
            hsl: {
                h: 0,
                s: 0,
                l: 73
            },
            name: "Grey74"
        },
        Grey78: {
            ID: 251,
            hex: "#c6c6c6",
            rgb: {
                r: 198,
                g: 198,
                b: 198
            },
            hsl: {
                h: 0,
                s: 0,
                l: 77
            },
            name: "Grey78"
        },
        Grey82: {
            ID: 252,
            hex: "#d0d0d0",
            rgb: {
                r: 208,
                g: 208,
                b: 208
            },
            hsl: {
                h: 0,
                s: 0,
                l: 81
            },
            name: "Grey82"
        },
        Grey85: {
            ID: 253,
            hex: "#dadada",
            rgb: {
                r: 218,
                g: 218,
                b: 218
            },
            hsl: {
                h: 0,
                s: 0,
                l: 85
            },
            name: "Grey85"
        },
        Grey89: {
            ID: 254,
            hex: "#e4e4e4",
            rgb: {
                r: 228,
                g: 228,
                b: 228
            },
            hsl: {
                h: 0,
                s: 0,
                l: 89
            },
            name: "Grey89"
        },
        Grey93: {
            ID: 255,
            hex: "#eeeeee",
            rgb: {
                r: 238,
                g: 238,
                b: 238
            },
            hsl: {
                h: 0,
                s: 0,
                l: 93
            },
            name: "Grey93"
        }
    };

    /**
     * 
     * @param colorID The number of the color in the XTerm pattern
     * from 0 to 255.
     */
    public XTermFromID(colorID: number): ColorPatterns {
        for(let color in this.XTerm) {
            let XTermColor = this.XTerm[color];
    
            if(XTermColor.ID === colorID) {
                return XTermColor;
            }
        }
    }
}
